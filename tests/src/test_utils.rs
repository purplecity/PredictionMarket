...
impl TestEnv {
	...
	pub async fn setup() -> anyhow::Result<Self> {
		// 加载 .env 文件
		let env_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().expect("parent dir exists").join("deploy/common.env");
		dotenv::from_path(&env_path).ok();

		// 从环境变量读取数据库配置
		let host = env::var("PREDICTION_EVENT_postgres_write_host").unwrap_or_else(|_| "127.0.0.1".to_string());
		let port = env::var("PREDICTION_EVENT_postgres_write_port").unwrap_or_else(|_| "5432".to_string()).parse::<u16>().expect("valid port number");
	...
	pub async fn create_test_event_and_market(&self, event_id: i64, market_id: i16) -> anyhow::Result<(String, String)> {
		...
		let market_data = MarketData {
			...
			question: format!("Will Test Market {} win?", market_id),
		...
		let market_json = serde_json::to_value(&markets_map)?;
...
pub fn parse_decimal(s: &str) -> Decimal {
	Decimal::from_str(s).expect("valid decimal string")
}
