use {
	crate::{consts::USER_EVENT_BATCH_SIZE, storage::UserStorage},
	common::{
		consts::{USER_EVENT_MSG_KEY, USER_EVENT_STREAM},
		redis_pool,
		websocket_types::UserEvent,
	},
...
async fn read_messages<C: redis::AsyncCommands>(conn: &mut C, storage: Arc<RwLock<UserStorage>>, last_id: &mut String) -> anyhow::Result<()> {
	// 使用 XREAD 批量读取消息
	let options = StreamReadOptions::default().count(USER_EVENT_BATCH_SIZE).block(0); // 一直阻塞直到收到消息
