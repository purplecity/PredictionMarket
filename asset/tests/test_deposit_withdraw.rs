...
use {rust_decimal::Decimal, test_utils::*};
...
async fn test_usdc_deposit() {
	...
	// 执行存款
	let result = asset::handlers::handle_deposit(user_id, token_id, amount, &trace_id, None, None, Some(privy_id.clone()), None, Some(env.pool.clone())).await;
...
async fn test_token_deposit() {
	...
	// 执行存款
	let result =
		asset::handlers::handle_deposit(user_id, &token_0_id, amount, &trace_id, Some(event_id), Some(market_id), Some(privy_id.clone()), Some("Yes".to_string()), Some(env.pool.clone())).await;
...
async fn test_usdc_withdraw() {
	...
	let withdraw_trace_id = uuid::Uuid::new_v4().to_string();

	// 创建测试用户
	env.create_test_user(user_id, &privy_id).await.expect("Failed to create test user");

	// 先存款
	asset::handlers::handle_deposit(user_id, token_id, deposit_amount, &deposit_trace_id, None, None, Some(privy_id.clone()), None, Some(env.pool.clone())).await.expect("Deposit should succeed");

	// 执行取款
	let result = asset::handlers::handle_withdraw(user_id, token_id, withdraw_amount, &withdraw_trace_id, None, None, Some(privy_id.clone()), None, Some(env.pool.clone())).await;
...
async fn test_duplicate_deposit_idempotency() {
	...
	// 第一次存款
	let result1 = asset::handlers::handle_deposit(user_id, token_id, amount, &trace_id, None, None, Some(privy_id.clone()), None, Some(env.pool.clone())).await;
	assert!(result1.is_ok(), "First deposit should succeed");

	// 使用相同的 trace_id 再次存款（应该失败，因为违反联合唯一约束）
	let result2 = asset::handlers::handle_deposit(user_id, token_id, amount, &trace_id, None, None, Some(privy_id.clone()), None, Some(env.pool.clone())).await;
