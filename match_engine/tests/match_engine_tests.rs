...
fn test_orderbook_add_order_symbol_mismatch() {
	...
	assert!(orderbook.add_order(order).is_err());
}
...
fn test_orderbook_best_bid() {
	...
	orderbook.add_order(create_test_order("order1", OrderSide::Buy, 6000, 100)).unwrap(); // 0.6
	orderbook.add_order(create_test_order("order2", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	orderbook.add_order(create_test_order("order3", OrderSide::Buy, 9330, 100)).unwrap(); // 0.933
...
fn test_orderbook_best_ask() {
	...
	// 添加多个卖单，价格从低到高
	orderbook.add_order(create_test_order("order1", OrderSide::Sell, 6000, 100)).unwrap(); // 0.6
	orderbook.add_order(create_test_order("order2", OrderSide::Sell, 5450, 100)).unwrap(); // 0.545
	orderbook.add_order(create_test_order("order3", OrderSide::Sell, 9330, 100)).unwrap(); // 0.933
...
fn test_orderbook_get_spread() {
	...
	// 使用浮点数价格测试
	orderbook.add_order(create_test_order("order1", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	orderbook.add_order(create_test_order("order2", OrderSide::Sell, 6330, 100)).unwrap(); // 0.633
...
fn test_orderbook_get_spread_no_bid() {
	...
	orderbook.add_order(create_test_order("order1", OrderSide::Sell, 600, 100)).unwrap();
...
fn test_orderbook_get_spread_no_ask() {
	...
	orderbook.add_order(create_test_order("order1", OrderSide::Buy, 500, 100)).unwrap();
...
fn test_orderbook_get_matching_orders_buy() {
	...
	// 添加卖单
	orderbook.add_order(create_test_order("sell1", OrderSide::Sell, 500, 100)).unwrap();
	orderbook.add_order(create_test_order("sell2", OrderSide::Sell, 600, 200)).unwrap();
	orderbook.add_order(create_test_order("sell3", OrderSide::Sell, 400, 150)).unwrap();
...
fn test_orderbook_get_matching_orders_sell() {
	...
	// 添加买单
	orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 500, 100)).unwrap();
	orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 600, 200)).unwrap();
	orderbook.add_order(create_test_order("buy3", OrderSide::Buy, 400, 150)).unwrap();
...
fn test_orderbook_get_depth() {
	...
	// 使用浮点数价格测试
	orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 9330, 200)).unwrap(); // 0.933
	orderbook.add_order(create_test_order("sell1", OrderSide::Sell, 7000, 150)).unwrap(); // 0.7
	orderbook.add_order(create_test_order("sell2", OrderSide::Sell, 8750, 250)).unwrap(); // 0.875
...
fn test_orderbook_get_depth_with_limit() {
	...
	// 添加多个价格档位
	for i in 1..=10 {
		orderbook.add_order(create_test_order(&format!("buy{}", i), OrderSide::Buy, (5000 + i * 100) as i32, 100)).unwrap();
	}
...
fn test_orderbook_get_depth_aggregates_quantity() {
	...
	// 同一价格档位多个订单
	orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 500, 100)).unwrap();
	orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 500, 200)).unwrap();
	orderbook.add_order(create_test_order("buy3", OrderSide::Buy, 500, 150)).unwrap();
...
fn test_orderbook_build_from_orders() {
	let symbol = create_test_symbol();
	let orders = vec![create_test_order("order1", OrderSide::Buy, 500, 100), create_test_order("order2", OrderSide::Sell, 600, 200)];
...
fn test_orderbook_get_orderbook_stats() {
	...
	orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 500, 100)).unwrap();
	orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 600, 200)).unwrap();
	orderbook.add_order(create_test_order("sell1", OrderSide::Sell, 700, 150)).unwrap();
...
async fn test_match_engine_submit_order_full_match() {
	...
	engine.token_0_orderbook.add_order(maker.clone()).unwrap();
	engine.token_0_orders.insert("maker1".to_string(), maker.clone());
...
async fn test_match_engine_cancel_order() {
	...
	engine.token_0_orderbook.add_order(order.clone()).unwrap();
	engine.token_0_orders.insert("order1".to_string(), order);
...
async fn test_match_engine_match_order_buy() {
	...
	engine.token_0_orderbook.add_order(maker1.clone()).unwrap();
	engine.token_0_orderbook.add_order(maker2.clone()).unwrap();
	engine.token_0_orders.insert("maker1".to_string(), maker1);
	engine.token_0_orders.insert("maker2".to_string(), maker2);
...
async fn test_match_engine_handle_snapshot_tick() {
	...
	// 添加一些订单（Yes结果）
	engine.token_0_orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	engine.token_0_orderbook.add_order(create_test_order("sell1", OrderSide::Sell, 6000, 150)).unwrap(); // 0.6
...
async fn test_match_engine_new_with_orders() {
	...
	assert_eq!(engine.token_0_orderbook.bids.len(), 1);
	assert_eq!(engine.token_1_orderbook.asks.len(), 1);
...
async fn test_match_engine_update_price_level_changes_new_level() {
	...
	// 第二次快照：添加新价格档位
	engine.token_0_orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 6000, 200)).unwrap(); // 0.6
	engine.handle_snapshot_tick();
...
async fn test_match_engine_update_price_level_changes_multiple_levels() {
	...
	engine.token_0_orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	engine.token_0_orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 6000, 200)).unwrap(); // 0.6
	engine.token_0_orderbook.add_order(create_test_order("sell1", OrderSide::Sell, 7000, 150)).unwrap(); // 0.7
	engine.handle_snapshot_tick();
	...
	engine.token_0_orderbook.update_order("buy1".to_string(), 50).unwrap();
	// 添加新价格档位
	engine.token_0_orderbook.add_order(create_test_order("buy3", OrderSide::Buy, 5500, 300)).unwrap(); // 0.55
	// 移除 sell1
...
async fn test_match_engine_update_price_level_changes_same_price_level_multiple_orders() {
	...
	// 第一次快照：同一价格档位多个订单
	engine.token_0_orderbook.add_order(create_test_order("buy1", OrderSide::Buy, 5000, 100)).unwrap(); // 0.5
	engine.token_0_orderbook.add_order(create_test_order("buy2", OrderSide::Buy, 5000, 200)).unwrap(); // 0.5
	engine.handle_snapshot_tick();
...
async fn test_match_engine_cross_matching_buy_yes_opposite_result() {
	...
	engine.token_1_orderbook.add_order(maker1.clone()).unwrap();
	engine.token_1_orderbook.add_order(maker2.clone()).unwrap();
	engine.token_1_orders.insert("maker1".to_string(), maker1);
	engine.token_1_orders.insert("maker2".to_string(), maker2);
...
async fn test_match_engine_cross_matching_buy_yes_both_results() {
	...
	engine.token_0_orderbook.add_order(yes_maker.clone()).unwrap();
	engine.token_0_orders.insert("yes_maker".to_string(), yes_maker);
	...
	engine.token_1_orderbook.add_order(no_maker.clone()).unwrap();
	engine.token_1_orders.insert("no_maker".to_string(), no_maker);
...
async fn test_match_engine_cross_matching_sell_opposite_result_price_limit() {
	...
	// 验证撮合后订单状态：maker2 应该被完全成交并从 no_orders 中移除
	// maker1 没有被匹配（因为taker_price < opposite_price），应该仍然存在
	assert!(engine.token_1_orders.contains_key("maker2"));
	assert!(!engine.token_1_orders.contains_key("maker1"));
	assert!(engine.token_1_orderbook.order_price_map.contains_key("maker2"));
	assert!(!engine.token_1_orderbook.order_price_map.contains_key("maker1"));
